...
`windows_mssql_bufman_extension_in_use_as_percentage` | Percentage of the buffer pool extension paging file occupied by buffer manager pages | counter | `mssql_instance`
...
`windows_mssql_memmgr_allocated_lock_owner_blocks` | The current number of allocated lock owner blocks | counter | `mssql_instance`
...
`windows_mssql_sqlstats_batch_requests` | Number of SQL batch requests received by server | counter | `mssql_instance`
`windows_mssql_sqlstats_failed_auto_parameterization_attempts` | Number of failed auto-parameterizations | counter | `mssql_instance`
