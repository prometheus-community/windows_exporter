...
func Listen(server *http.Server, tlsConfigPath string) error {
	if tlsConfigPath == "" {
		log.Infoln("No TLS config provided. TLS & HTTP2 are disabled.")
	...
	case nil:
		...
		// Valid TLS config.
		log.Infoln("TLS is enabled.")
		log.Infoln("HTTP2 enabled: ", c.HTTPConfig.HTTP2)
	case errNoTLSConfig:
		// No TLS config, back to plain HTTP.
		log.Infoln("TLS is disabled")
		log.Infoln("HTTP2 is disabled")
		return server.ListenAndServe()
	default:
		// Invalid TLS config.
		return err
