...
var (
	...
	ErrorMap = map[string]*regexp.Regexp{
		...
		"No such file":                 regexp.MustCompile(`The system cannot find the file specified`),
...
func TestConfigReloading(t *testing.T) {
	...
	if err == nil {
		...
		if err = swapFileContents(goodYAMLPath, badYAMLPath); err != nil {
			recordConnectionError(fmt.Errorf("failed swapping file contents: %s", err))
		}
		err = TestClientConnection()
		if err != nil {
