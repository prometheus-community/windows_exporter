...
func (c *Collector) Build(_ *slog.Logger, _ *wmi.Client) error {
	...
	c.perfDataCollector4, err = perfdata.NewCollector(perfdata.Registry, "TCPv4", nil, counters)
	if err != nil {
		return fmt.Errorf("failed to create TCPv4 collector: %w", err)
	}

	c.perfDataCollector6, err = perfdata.NewCollector(perfdata.Registry, "TCPv6", nil, counters)
	if err != nil {
...
func (c *Collector) collect(ch chan<- prometheus.Metric) error {
	...
	c.writeTCPCounters(ch, data[pdh.EmptyInstance], []string{"ipv4"})
	...
	c.writeTCPCounters(ch, data[pdh.EmptyInstance], []string{"ipv6"})

	return nil
}

func (c *Collector) writeTCPCounters(ch chan<- prometheus.Metric, metrics map[string]pdh.CounterValues, labels []string) {
