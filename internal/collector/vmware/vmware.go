...
func (c *Collector) collectMem(ch chan<- prometheus.Metric) error {
	...
	if err := c.miSession.Query(dst, mi.NamespaceRootCIMv2, "SELECT * FROM Win32_PerfRawData_vmGuestLib_VMem"); err != nil {
...
func (c *Collector) collectCpu(ch chan<- prometheus.Metric) error {
	...
	if err := c.miSession.Query(dst, mi.NamespaceRootCIMv2, "SELECT * FROM Win32_PerfRawData_vmGuestLib_VCPU"); err != nil {
		return fmt.Errorf("WMI query failed: %w", err)
	}
