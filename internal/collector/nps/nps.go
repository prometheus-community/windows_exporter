...
func (c *Collector) Build(_ *slog.Logger, _ *mi.Session) error {
	...
	c.accessPerfDataCollector, err = pdh.NewCollector[perfDataCounterValuesAccess](pdh.ResultTypeRaw, "NPS Authentication Server", nil)
	if err != nil {
		errs = append(errs, fmt.Errorf("failed to create NPS Authentication Server collector: %w", err))
	}

	c.accountingPerfDataCollector, err = pdh.NewCollector[perfDataCounterValuesAccounting](pdh.ResultTypeRaw, "NPS Accounting Server", nil)
	if err != nil {
