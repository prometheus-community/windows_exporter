...
func (c *Collector) Build(_ *slog.Logger, _ *wmi.Client) error {
	if utils.PDHEnabled() {
		...
		c.perfDataCollector, err = perfdata.NewCollector(perfdata.Registry, "AD FS", perfdata.AllInstances, counters)
		if err != nil {
...
func (c *Collector) collect(ctx *types.ScrapeContext, logger *slog.Logger, ch chan<- prometheus.Metric) error {
	...
	err := registry.UnmarshalObject(ctx.PerfObjects["AD FS"], &adfsData, logger)
	if err != nil {
		return err
	}
