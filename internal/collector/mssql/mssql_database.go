...
func (c *Collector) buildDatabases() error {
	...
	for _, sqlInstance := range c.mssqlInstances {
		c.databasesPerfDataCollectors[sqlInstance.name], err = pdh.NewCollector[perfDataCounterValuesDatabases](pdh.ResultTypeRaw, c.mssqlGetPerfObjectName(sqlInstance.name, "Databases"), pdh.InstancesAll)
		if err != nil {
		...
		if sqlInstance.isVersionGreaterOrEqualThan(serverVersion2019) {
			c.databasesPerfDataCollectors2019[sqlInstance.name], err = pdh.NewCollector[perfDataCounterValuesDatabases2019](pdh.ResultTypeRaw, c.mssqlGetPerfObjectName(sqlInstance.name, "Databases"), pdh.InstancesAll)
			if err != nil {
