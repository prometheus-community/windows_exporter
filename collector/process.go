...
func newProcessCollector() (Collector, error) {
	...
	return &processCollector{
		StartTime: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "service_display_name"},
			nil,
		),
		CPUTimeTotal: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "mode", "service_display_name"},
			nil,
		),
		...
		PoolBytes: prometheus.NewDesc(
			...
			[]string{"process", "process_id", "creating_process_id", "pool", "service_display_name"},
			nil,
		),
...
func (c *processCollector) Collect(ctx *ScrapeContext, ch chan<- prometheus.Metric) error {
	...
	// Query service list to provide display name for processess if it is part of a service.
	var dst_svc []Win32_Service
	var svcDisplayName string
	q_svc := queryAll(&dst_svc)
	if err := wmi.Query(q_svc, &dst_svc); err != nil {
		return err
	}
