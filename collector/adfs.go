...
func newADFSCollector() (Collector, error) {
	...
	return &adfsCollector{
		...
		avgArtifactDBQueryTime: prometheus.NewDesc(
			prometheus.BuildFQName(Namespace, subsystem, "db_artifact_query_average_time_seconds"),
			"Average time for each artifact database query",
			nil,
			nil,
		),
		...
		avgConfigDBQueryTime: prometheus.NewDesc(
			prometheus.BuildFQName(Namespace, subsystem, "db_config_query_average_time_seconds"),
			"Average time for each configuration database query",
			nil,
			nil,
		),
